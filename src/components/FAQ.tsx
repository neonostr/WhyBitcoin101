...
const NostrQuestionModal = () => {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-primary text-primary-foreground hover:bg-primary/90">Ask the Community</Button>
      </DialogTrigger>
      <DialogContent className="max-w-md">
        ...
        {!showSuccess ? <div className="space-y-4">
            ...
            <Textarea placeholder="What's your Bitcoin question?" value={question} onChange={e => setQuestion(e.target.value)} className="min-h-[100px]" maxLength={280} />
            
            <p className="text-xs text-muted-foreground">
              No account needed • Anonymous posting • Decentralized network
            </p>
            
            <Button onClick={submitQuestion} disabled={isSubmitting || !question.trim()} className="w-full bg-primary text-primary-foreground hover:bg-primary/90">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <input type="text" value={followUpLink} readOnly className="flex-1 px-3 py-2 text-xs bg-background border border-border rounded-md" />
            ...
            <Button onClick={resetModal} className="w-full bg-primary text-primary-foreground hover:bg-primary/90">
              Close
            </Button>
          </div>}
      </DialogContent>
...
const FAQ = () => {
  ...
  return <section id="faq" className="py-20 px-4 bg-gradient-to-b from-background to-muted/20">
      <div className="container mx-auto">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="grid md:grid-cols-2 gap-6">
            {faqs.map((faq, index) => <Card key={index} className="group hover:shadow-[var(--card-hover)] transition-all duration-300 border-border/50 hover:border-primary/20">
                <CardHeader>
