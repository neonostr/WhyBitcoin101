...
const DonationModal = ({ isOpen, onClose }: DonationModalProps) => {
  ...
  const [qrCodes, setQrCodes] = useState<{ lightning: string | null; onchain: string | null }>({
    lightning: null,
    onchain: null
  });
  ...
  const generateQR = async (text: string, type: 'lightning' | 'onchain') => {
    try {
      ...
      setQrCodes(prev => ({ ...prev, [type]: qrDataUrl }));
    } catch (err) {
    ...
    <div className="space-y-4 p-6 rounded-lg border border-border bg-card">
      ...
      <div className="space-y-3">
        ...
        <Button
          variant="secondary"
          size="sm"
          onClick={() => generateQR(address, type)}
          className="w-full"
        >
        ...
        <div className="space-y-6">
          <div className="text-center">
            <p className="text-muted-foreground">
              Help us continue providing free Bitcoin education resources. 
              Your support is appreciated but never required.
            </p>
          </div>
          ...
          <div className="grid gap-6">
            <PaymentOption
              ...
              description="Fast and low-fee payments via Lightning Network"
              type="lightning"
            />
            ...
            <PaymentOption
              ...
              description="Traditional Bitcoin network payment"
              type="onchain"
            />
          </div>
